-- Registro do horário de saída para rota + transição atômica de status

ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS em_rota_at timestamptz;

-- Marca o horário sempre que o pedido entra em rota, por qualquer caminho de atualização
...
-- Coloca pedidos em rota de forma atômica por pedido: baixa de estoque e mudança
-- de status acontecem juntas ou não acontecem. Uma falha em um pedido não impede
-- os demais; o resultado de cada um é retornado para o chamador.
