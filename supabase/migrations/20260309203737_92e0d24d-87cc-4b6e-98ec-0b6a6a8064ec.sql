...
CREATE OR REPLACE FUNCTION public.create_full_site_order(p_customer_name text, p_customer_phone text, p_customer_type customer_type DEFAULT 'PF'::customer_type, p_customer_cnpj text DEFAULT NULL::text, p_street text DEFAULT ''::text, p_number text DEFAULT ''::text, p_neighborhood text DEFAULT ''::text, p_city text DEFAULT 'Santo André'::text, p_state text DEFAULT 'SP'::text, p_complement text DEFAULT NULL::text, p_zip text DEFAULT NULL::text, p_notes text DEFAULT NULL::text, p_delivery_date date DEFAULT NULL::date, p_delivery_time text DEFAULT NULL::text, p_items jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- On match, reuse existing record WITHOUT overwriting the name
