      ...
      <main className="flex-1 container py-8">
        ...
        {filtered.length === 0 ? (
          <p className="text-center text-muted-foreground py-12">
            Nenhum produto encontrado. Em breve adicionaremos novos itens!
          </p>
        ) : (
            ...
            {filtered.map((p) => (
              <Card key={p.id} className="flex flex-col">
                <CardHeader className="flex-row items-center gap-3">
                  {iconMap[p.icon] || <Droplets className="h-8 w-8 text-primary" />}
                  <div>
                ...
                <CardContent className="flex-1">
                  ...
                  <p className="mt-3 font-semibold text-primary">{p.priceText}</p>
                </CardContent>
                <CardFooter>
                  <Button
                  ...
                  >
                    <a
                      href={business.waLink(`Olá! Tenho interesse no produto: ${p.name}`)}
                      target="_blank"
