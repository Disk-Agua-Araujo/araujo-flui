                ...
                <div className="mt-4 space-y-3">
                  {items.map((item) => (
                    <div key={item.product.id} className="flex items-center gap-3 border rounded-md p-3">
                      ...
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => updateQty(item.product.id, item.qty - 1)}>
                  ...
                  <form onSubmit={handleCheckout} className="space-y-3 pt-4 border-t">
                    ...
                    <div>
                      ...
                      <Input id="c-wa" type="tel" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} required />
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <div className="col-span-2">
                        <Label htmlFor="c-rua">Rua *</Label>
                        <Input id="c-rua" value={rua} onChange={(e) => setRua(e.target.value)} required />
                      </div>
                      <div>
                        <Label htmlFor="c-num">Nº *</Label>
                        <Input id="c-num" value={numero} onChange={(e) => setNumero(e.target.value)} required />
                      </div>
                    </div>
                    <div>
                      <Label htmlFor="c-bairro">Bairro *</Label>
                      <Input id="c-bairro" value={bairro} onChange={(e) => setBairro(e.target.value)} required />
                    </div>
                    <div>
                      <Label htmlFor="c-comp">Complemento</Label>
                      <Input id="c-comp" value={complemento} onChange={(e) => setComplemento(e.target.value)} />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Textarea id="c-obs" value={obs} onChange={(e) => setObs(e.target.value)} rows={2} />
                    </div>
                    <div className="text-xs text-muted-foreground flex items-center gap-1">
                      <MapPin className="h-3 w-3" /> Entregamos até {MAX_DELIVERY_KM}km
                    </div>
          ...
          {retailProducts.map((p) => (
            <Card key={p.id} className="flex flex-col">
              <CardHeader className="flex-row items-center gap-3">
                {iconMap[p.icon] || <Droplets className="h-7 w-7 text-primary" />}
                <CardTitle className="text-lg">{p.name}</CardTitle>
              </CardHeader>
              <CardContent className="flex-1">
                <p className="text-sm text-muted-foreground">{p.description}</p>
                <p className="mt-2 font-semibold text-primary">{p.priceText}</p>
              </CardContent>
              <CardFooter>
                <Button className="w-full" onClick={() => { addItem(p); toast({ title: `${p.name} adicionado ao carrinho` }); }}>
